...
export default function Schedule({ schedule }) {
  ...
  return (
    <div
      className={
        "schedule main__column main__column--1 main__background main__column--schedule" +
            ...
            {Object.entries(schedule[0]).map(([key], index) => {
              return (
                key !== "id" && (
                  <div
                    className={
                      "table__text table__column table__column--" + (index + 1)
                    }
                    key={index}
                  >
                    {key}
                  ...
                  {Object.entries(item).map(([key, value], index) => {
                    return (
                      key !== "id" && (
                        <div
                          className={
                            "table__column table__column--" + (index + 1)
                          }
                          key={index}
                        >
